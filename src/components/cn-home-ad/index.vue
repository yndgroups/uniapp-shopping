<template>
  ...
  >
    ...
    >
      ...
      ></cn-img>
      <view v-if="title || item.title" class="title">
        {{ item.title }}
      </view>
    </swiper-item>
  </swiper>
  <view
    class="item-box-shadow"
    :style="{
      background: `url( ${urls(lists)[state.index]})`,
    }"
  >
  </view>
</template>
...
function change(e: any) {
  ...
  // if (e && e.targetUrl && e.targetUrl.includes("?")) {
  //   let list = e.targetUrl.split("?");
  //   uni.navigateTo({
  //     url: `/pages/goods-details/index?${list[1]}`,
  //   });
  // }
}
